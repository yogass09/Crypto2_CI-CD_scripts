...
# FETCHER
# all_coins_historical <- crypto_history(coin_list = coin_list_all,convert = "USD", limit = 10000,sleep = 0, interval= "1h")

# REFRESHER
Hourly_OHLCV<-crypto_history(coin_list = crypto.list,
                                      convert = "USD", 
                                      limit = 1,sleep = 0,
                                      start_date = Sys.Date()-1,
                                      end_date = Sys.Date()+1,interval="1h")
...
dbWriteTable(con, "Hourly_OHLCV", as.data.frame(Hourly_OHLCV), append = TRUE)
